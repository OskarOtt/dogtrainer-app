...
/**
 * Dog picker for the "Add Goal" flow. Auto-selects (and skips itself)
 * when the user has only one dog, otherwise lets them choose which dog
 * the new goal is for before opening the goal form.
 */

export default function PickDogForGoalScreen() {
  ...
  useEffect(() => {
    if (!redirected.current && dogs && dogs.length === 1) {
      ...
      router.replace(`/dog/${dogs[0].id}/goals/new`);
    }
  }, [dogs, router]);
    ...
    <ThemedView style={styles.container}>
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => <DogCard dog={item} onPress={() => router.push(`/dog/${item.id}/goals/new`)} />}
        />
      </SafeAreaView>
    </ThemedView>
  );
